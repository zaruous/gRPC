...
public class ServerReflectionClient {
	...
	private static final Logger logger = Logger.getLogger(ServerReflectionClient.class.getName());
	...
	/**
	 * Asks the remote server to list its services and completes when the server
	 * responds.
	 */
	public ListenableFuture<ImmutableList<String>> listServices() {
		...
		StreamObserver<ServerReflectionRequest> requestStream = ServerReflectionGrpc.newStub(channel)
				.withDeadlineAfter(LIST_RPC_DEADLINE_MS, TimeUnit.MILLISECONDS).serverReflectionInfo(rpcHandler);
		return rpcHandler.start(requestStream);
	}

	/**
	 * Returns a {@link FileDescriptorSet} containing all the transitive
	 * dependencies of the supplied service, as provided by the remote server.
	 */
	public ListenableFuture<FileDescriptorSet> lookupService(String serviceName) {
		...
		StreamObserver<ServerReflectionRequest> requestStream = ServerReflectionGrpc.newStub(channel)
				.withDeadlineAfter(LOOKUP_RPC_DEADLINE_MS, TimeUnit.MILLISECONDS).serverReflectionInfo(rpcHandler);
		return rpcHandler.start(requestStream);
	}
	...
	private static class ListServicesHandler implements StreamObserver<ServerReflectionResponse> {
		...
		public void onNext(ServerReflectionResponse serverReflectionResponse) {
			...
			default:
				...
				logger.warning("Got unknown reflection response type: " + responseCase);
				break;
			}
		}
		...
		public void onCompleted() {
			if (!resultFuture.isDone()) {
				// Use logger.severe instead of logger.error
				logger.severe("Unexpected completion of server reflection rpc while listing services");
	...
	private static class LookupServiceHandler implements StreamObserver<ServerReflectionResponse> {
		...
		// Used to notice when we've received all the files we've asked for and we can
		// end the rpc.
		...
		ListenableFuture<FileDescriptorSet> start(StreamObserver<ServerReflectionRequest> requestStream) {
			this.requestStream = requestStream;
		...
		public void onNext(ServerReflectionResponse response) {
			...
			case FILE_DESCRIPTOR_RESPONSE:
				ImmutableSet<FileDescriptorProto> descriptors = parseDescriptors(
		...
		public void onCompleted() {
			if (!resultFuture.isDone()) {
				// Use logger.severe instead of logger.error
				logger.severe("Unexpected completion of the server reflection rpc");
				resultFuture.setException(new RuntimeException("Unexpected end of rpc"));
			}
		}
		...
		private ImmutableSet<FileDescriptorProto> parseDescriptors(List<ByteString> descriptorBytes) {
			...
			for (ByteString fileDescriptorBytes : descriptorBytes) {
				try {
					...
					// Use logger.warning instead of logger.warn
					logger.log(Level.WARNING, "Failed to parse bytes as file descriptor proto", e);
				}
			}
		...
		private void processDependencies(FileDescriptorProto fileDescriptor) {
			// Use logger.fine instead of logger.debug
			logger.fine("Processing deps of descriptor: " + fileDescriptor.getName());
			...
			if (outstandingRequests == 0) {
				// Use logger.fine instead of logger.debug
				logger.fine("Retrieved service definition for [" + serviceName + "] by reflection");
				resultFuture.set(FileDescriptorSet.newBuilder().addAllFile(resolvedDescriptors.values()).build());
				requestStream.onCompleted();
			}
		}
